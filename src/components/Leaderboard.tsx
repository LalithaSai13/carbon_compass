...
interface UserEmissions {
  userId: string; // This will be the user's email (document ID)
  userName: string;
...
const Leaderboard: React.FC = () => {
  ...
  const fetchLeaderboardData = async () => {
    try {
      ...
      const userEmissions: { [key: string]: { userName: string; cumulativeEmissions: number; submissionCount: number } } = {};

      // Fetch emissions data
      snapshot.forEach((doc) => {
        const data = doc.data();
        
        const userId = doc.id; // Use document ID (user's email) as user ID
        const userName = data.name || 'Unknown User'; // Extract the name field
        const submissions = data.submissions || []; // Ensure submissions is an array
        
        const cumulativeEmissions = submissions.reduce((acc: number, submission: any) => {
          return acc + (parseFloat(submission.totalEmissions) || 0); // Accumulate total emissions from each submission
        }, 0);

        const submissionCount = submissions.length; // Count submissions

        // Store the user emissions data
      ...
      const leaderboard: UserEmissions[] = Object.entries(userEmissions).map(
        ([userId, { userName, cumulativeEmissions, submissionCount }]) => {
          const average = submissionCount > 0 ? cumulativeEmissions / submissionCount : 0; // Avoid division by zero
          return { userId, userName, averageEmissions: average, cumulativeEmissions, submissionCount };
        }
      );

      // Sort leaderboard by average emissions in ascending order
